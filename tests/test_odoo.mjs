...
async function test() {
    ...
    try {
        ...
        // In Odoo 19, result is the uid directly (not {uid: ...})
        if (result.result && typeof result.result === 'number' && result.result > 0) {
            console.log('\n✓ Authentication SUCCESSFUL!');
            console.log(`UID: ${result.result}`);
        } else if (typeof result.result === 'number' && result.result > 0) {
            console.log('\n✓ Authentication SUCCESSFUL!');
            console.log(`UID: ${result.result}`);
        } else {
